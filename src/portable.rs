...
const DEFAULT_TTA_CONTEXTNET_BIN: &'static [u8] =
    include_bytes!("../target/models/rife-v3.1/contextnet.bin");
const DEFAULT_TTA_CONTEXTNET_PARAM: &str =
    include_str!("../target/models/rife-v3.1/contextnet.param");
const DEFAULT_TTA_FLOWNET_BIN: &'static [u8] =
    include_bytes!("../target/models/rife-v3.1/flownet.bin");
const DEFAULT_TTA_FLOWNET_PARAM: &str = include_str!("../target/models/rife-v3.1/flownet.param");
const DEFAULT_TTA_FUSIONNET_BIN: &'static [u8] =
    include_bytes!("../target/models/rife-v3.1/fusionnet.bin");
const DEFAULT_TTA_FUSIONNET_PARAM: &str =
    include_str!("../target/models/rife-v3.1/fusionnet.param");
...
pub fn get_local_path() -> PathBuf {
    ...
    if cfg!(target_os = "windows") || is_portable() {
        ...
        if !local_path.exists() {
            fs::create_dir_all(&local_path).expect("Failed to create local folder");
        }
    }
...
pub fn get_default_model_tta_path() -> PathBuf {
    let model_path = get_local_path().join("models/rife-v3.1");
    if !model_path.exists() {
        fs::create_dir_all(&model_path).unwrap();
        fs::write(model_path.join("flownet.bin"), DEFAULT_TTA_FLOWNET_BIN).unwrap();
        fs::write(model_path.join("flownet.param"), DEFAULT_TTA_FLOWNET_PARAM).unwrap();
        fs::write(
            model_path.join("contextnet.bin"),
            DEFAULT_TTA_CONTEXTNET_BIN,
        )
        .unwrap();
        fs::write(
            model_path.join("contextnet.param"),
            DEFAULT_TTA_CONTEXTNET_PARAM,
        )
        .unwrap();
        fs::write(model_path.join("fusionnet.bin"), DEFAULT_TTA_FUSIONNET_BIN).unwrap();
        fs::write(
            model_path.join("fusionnet.param"),
            DEFAULT_TTA_FUSIONNET_PARAM,
        )
        .unwrap();
    } else if !model_path.join("flownet.bin").exists()
        || !model_path.join("flownet.param").exists()
        || !model_path.join("contextnet.bin").exists()
        || !model_path.join("contextnet.param").exists()
        || !model_path.join("fusionnet.bin").exists()
        || !model_path.join("fusionnet.param").exists()
    {
        fs::write(model_path.join("flownet.bin"), DEFAULT_TTA_FLOWNET_BIN).unwrap();
        fs::write(model_path.join("flownet.param"), DEFAULT_TTA_FLOWNET_PARAM).unwrap();
        fs::write(
