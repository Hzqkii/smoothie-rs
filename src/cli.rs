...
If you'd like help, take a screenshot of this message and your recipe and come over to discord.gg/CTT and make a post in #support
    ...
    let home_dir = env::home_dir().expect("How do you not have a user dir?");
    let config_path = home_dir.join(".config/smoothie-rs");
    if !config_path.exists() {
        panic!("expected folder @ {}", config_path.display());
    }
    let local_path = home_dir.join(".local/share/smoothie-rs");
    ...
    if !last_args.exists() {
        if File::create(&last_args).is_err() {
            panic!("Failed to create last_args.txt at {}", last_args.display())
        };
    }
